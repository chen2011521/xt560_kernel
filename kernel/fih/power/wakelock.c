...
/* FIHTDC, Div2-SW2-BSP, Penho, SuspendLog { */
#ifdef CONFIG_FIH_SUSPEND_RESUME_LOG
		ret += seq_printf(m, "<< *active_wake_locks[WAKE_LOCK_%s] >> :\n", (type == 0) ? "SUSPEND" : "IDLE");
#endif	// CONFIG_FIH_SUSPEND_RESUME_LOG
/* } FIHTDC, Div2-SW2-BSP, Penho, SuspendLog */
...
static void print_active_locks(int type)
{
	...
	list_for_each_entry(lock, &active_wake_locks[type], link) {
		if (lock->flags & WAKE_LOCK_AUTO_EXPIRE) {
			...
			if (timeout > 0)
				pr_info("[PW] active wake lock %s, time left %ld\n",
					lock->name, timeout);
			else if (print_expired)
				pr_info("[PW] wake lock %s, expired\n", lock->name);
		} else {
			pr_info("[PW] active wake lock %s\n", lock->name);
...
#ifdef CONFIG_FIH_7X27A_PROJS
long _has_wake_lock(const char * caller, int type)
#else	// CONFIG_FIH_7X27A_PROJS
...
#ifdef CONFIG_FIH_7X27A_PROJS
	{
		printk(KERN_INFO "[PW] %s has_wake_lock :\n", caller);
...
#ifdef CONFIG_FIH_7X27A_PROJS
			{
				if ((!isInSwqTrace()) && (debug_mask & DEBUG_SUSPEND))
					printk(KERN_INFO "[PW] last wake_lock : %s\n", lock->name);
				queue_work(suspend_work_queue, &suspend_work);
			}
#else	// CONFIG_FIH_7X27A_PROJS
				queue_work(suspend_work_queue, &suspend_work);

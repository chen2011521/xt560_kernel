...
/* set debugmask to open/close printk */
static uint32_t printk_debug_mask = 1;
module_param_named(debug_mask, printk_debug_mask, uint, 0644);
...
static int syslog_action_restricted(int type)
{
	...
	return type != SYSLOG_ACTION_READ_ALL && type != SYSLOG_ACTION_SIZE_BUFFER && type != SYSLOG_ACTION_READ;/*FIHTDC DerrickDRLiu DbgCfgTool */
}
...
/* add flog */
#if defined(CONFIG_FIH_FLOG)
int do_ftmlog(int type, char __user *buf, int len, bool from_file)
{
	unsigned i;
	char c;
	int error;
...
asmlinkage int printk(const char *fmt, ...)
{
	...
	/* set debugmask to open/close printk */
	/* no close printk when kernel panic by PRINTK_DEBUG_MASK_OFFSET from panic() */
#ifndef CONFIG_FIH_FTM
	if ((printk_debug_mask == 0)&&(*(uint32_t *)PRINTK_DEBUG_MASK_OFFSET == 0)) {
		return 0;
	}
...
/* FIH, KanyYJLin, debugmask, 2011/09/22 { */
//#ifdef CONFIG_FIH_FXX
	/* initial debugmask to open/close printk */
	printk_debug_mask = *(uint32_t *)PRINTK_DEBUG_MASK_OFFSET;
//#endif
/* FIH, KanyYJLin, debugmask, 2011/09/22 } */
...
/* FIH, CHHsieh, 2011/10/31 { */
/* save kernel log into flash when resume */
#ifdef CONFIG_FIH_FTM
char *klogd_buf_addr(void)
{
	return __log_buf;
}
EXPORT_SYMBOL(klogd_buf_addr);
#endif //CONFIG_FIH_FTM
/* FIH, CHHsieh, 2011/10/31 } */

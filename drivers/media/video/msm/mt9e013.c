...
//vince, OTP +++
//static ssize_t mt9e013_otp_check(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
static int mt9e013_otp_check(void)
{
	uint16_t counter = 0;
	...
	mt9e013_i2c_read(0x301A, &R0x301A, 2); //RESET_REGISTER_REG_ED_EN
	mt9e013_i2c_read(0x3134, &R0x3134, 2);
	mt9e013_i2c_read(0x304C, &R0x304C, 2);
	mt9e013_i2c_read(0x304A, &R0x304A, 2);
	...
	for(i=3; i>=0; i--)//from 4th PAGE
	{
		...
		mt9e013_i2c_write_w_sensor(0x301A, R0x301A | 0x0020);
		mt9e013_i2c_write_w_sensor(0x301A, 0x0610); //disable streaming
		mt9e013_i2c_write_w_sensor(0x3134, 0xCD95); //timing paramerters for otp read
		//mt9e013_i2c_write_w_sensor(0x304C, 0x3000); //record type 0x30
		mt9e013_i2c_write_w_sensor(0x304C, page_reg[i]); //record type 0x30
		mt9e013_i2c_write_w_sensor(0x304A, 0x0010); //only read single record at a time
		counter=0;
		...
		mt9e013_i2c_read(0x38D4, &value, 2);
		if((value&0xff)==0xff)
			break;
		else
	...
	mt9e013_i2c_read(0x38D4, &checksum, 2);
...
static int read_LSC(void)
{
	...
	for(i=0;i<106;i++)
	{
		rc = mt9e013_i2c_read(mt9e013_regs.reg_lsc[i].waddr, &value, 2);	
		printk("R0x%x = %x\n",mt9e013_regs.reg_lsc[i].waddr,value);	
	}
	printk("------read_LSC-------\n");
	return rc;
}
...
static int write_OTP_to_LSC(void)
{
	...
	{
		mt9e013_i2c_read(i, &value, 2); 
		mt9e013_regs.reg_lsc[j].wdata = value;
		j++;
	}
	rc = mt9e013_i2c_write_w_table(mt9e013_regs.reg_lsc,mt9e013_regs.reg_lsc_size);
	//enable OTP
...
static ssize_t mt9e013_LSC_read(struct device *dev, struct device_attribute *attr, char *buf)
{
	read_LSC();
	return 0;
}
DEVICE_ATTR(mt9e013_lsc_table, 0664, mt9e013_LSC_read, NULL);
static ssize_t mt9e013_OTP_read(struct device *dev, struct device_attribute *attr, char *buf)
{
	read_OTP_value();
	return 0;
}
DEVICE_ATTR(mt9e013_otp_table, 0664, mt9e013_OTP_read, NULL);
static int mt9e013_create_attributes(struct i2c_client *client)
{
	int rc;

	rc = device_create_file(&client->dev, &dev_attr_mt9e013_lsc_table);
	rc = device_create_file(&client->dev, &dev_attr_mt9e013_otp_table);

	if (rc < 0) {
		dev_err(&client->dev, "%s: Create mt9e013 attribute  failed!! <%d>", __func__, rc);	
		return rc; 
	}
...
static int32_t mt9e013_sensor_setting(int update_type, int rt)
{
	...
	if (update_type == REG_INIT) {
		...
		if(!mt9e013_otp_check())
		{
			printk("mt9e013: read OTP completed\n");
			if(!write_OTP_to_LSC())
				printk("mt9e013: write OTP to LSC completed\n");
			else
				printk("mt9e013: write OTP to LSC fail\n");
		}
		else
			printk("mt9e013: read OTP fail\n");
		cam_debug_init();
		CSI_CONFIG = 0;
	...
	int res)
{
	...
	case SENSOR_HFR_120FPS_MODE:
		...
		rc = mt9e013_set_default_focus(0);
		if(!rc)
			printk("set mt9e013_set_default_focus after start preview!!\n");
		else
			printk("set mt9e013_set_default_focus fail!!\n");
		break;
...
static int mt9e013_probe_init_sensor(const struct msm_camera_sensor_info *data)
{
	...
	CDBG("ID: %d\n", chipid);
		...
		msleep(10);
                rc = mt9e013_i2c_read(0x0000, &chipid, 2);
                count++;
                printk("mt9e013_probe_init_sensor: retry %d, chipid=0x%x\n",count,chipid);                
        }
        if (chipid != 0x4B00)
...
int mt9e013_sensor_open_init(const struct msm_camera_sensor_info *data)
{
    ...
    /* FIH, Ming, AF { */
	/* enable AF actuator */
	if (mt9e013_ctrl->sensordata->vcm_enable) {
		CDBG("enable AF actuator, gpio = %d\n",
			 mt9e013_ctrl->sensordata->vcm_pwd);
		rc = gpio_request(mt9e013_ctrl->sensordata->vcm_pwd,
						"mt9e013");
		if (!rc)
			gpio_direction_output(
				mt9e013_ctrl->sensordata->vcm_pwd, 1);
		else {
			CDBG("mt9e013_ctrl gpio request failed!\n");
			goto init_fail;
		}
	}
	/* } FIH, Ming, AF */
...
static int mt9e013_sensor_release(void)
{
	...
	/* FIH, Ming, AF { */
	if (mt9e013_ctrl->sensordata->vcm_enable) {
		gpio_direction_output(mt9e013_ctrl->sensordata->vcm_pwd, 0);
		gpio_free(mt9e013_ctrl->sensordata->vcm_pwd);
	}
	/* } FIH, Ming, AF */
		...
		struct msm_sensor_ctrl *s)
{
	...
	mt9e013_create_attributes(mt9e013_client);
	return rc;
...
/* FIH, Ming, Version Control { */
#ifndef CONFIG_USE_QUALCOMM_DEVICES 
#include "fih_mt9e013.c"
#else

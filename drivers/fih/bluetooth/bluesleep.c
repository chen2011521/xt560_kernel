...
struct bluesleep_info {
	unsigned host_wake;
	unsigned ext_wake;
#if defined(CONFIG_BROADCOM_BCM4330_BLUETOOTH)
	int host_wake_irq;
#else
...
{
	printk("[TT] bluesleep_sleep_wakeup");
...
static void bluesleep_sleep_work(struct work_struct *work)
{
	if (bluesleep_can_sleep()) {
		...
		if (test_bit(BT_ASLEEP, &flags)) {
			printk("[TT]already asleep\n");
			return;
		}

		if (msm_hs_tx_empty(bsi->uport)) {
			printk("[TT]going to sleep...\n");
			printk("[TT]set_bit: BT_ASLEEP\n");
...
static void bluesleep_outgoing_data(void)
{
	...
	printk("[TT]bluesleep_outgoing_data\n");
	/* log data passing by */
	printk("[TT]set_bit: BT_TXDATA\n");
...
static void bluesleep_tx_timer_expire(unsigned long data)
{
	...
	printk("[TT]Tx timer expired\n");

	/* were we silent during the last timeout? */
	if (!test_bit(BT_TXDATA, &flags)) {
		printk("[TT]Tx has been idle\n");
#if defined(CONFIG_BROADCOM_BCM4330_BLUETOOTH)
		printk("[TT]set_value bsi->ext_wake 0\n");
		gpio_set_value(bsi->ext_wake, 0);
#else
		gpio_set_value(bsi->ext_wake, 1);
#endif
		...
		printk("[TT]Tx data during last period\n");
		mod_timer(&tx_timer, jiffies + (TX_TIMER_INTERVAL*HZ));
	}

	/* clear the incoming data flag */
	printk("[TT]clear_bit: BT_TXDATA\n");

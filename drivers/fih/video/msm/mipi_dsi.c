...
/* The backlight control  apply on TBP and TQW*/
#ifdef CONFIG_FIH_7X27A_PROJS
extern char * read_hw_cfg(void);
extern void lcd_backlight_trigger_toshiba(void);
extern void lcd_backlight_trigger_renesas(void);
extern void register_lcd_backlight_trigger(backlight_trigger_t backlight_trigger);
#endif
/* } FIHNJDC, SW4-BSP Jenny, 2011/10/12 */
...
static int mipi_dsi_on(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "[DISPLAY] mipi_dsi_on : CLK_STATUS(0x%08x)\n", MIPI_INP(MIPI_DSI_BASE + 0x11c));
...
static int mipi_dsi_probe(struct platform_device *pdev)
{
	...
	if ((pdev->id == 1) && (pdev->num_resources >= 0)) {
        ...
        if((*(versionCtrl+DRIVER_TYPE_LCM)==LCM_TOSHIBA_UPD161809_V1) || 
           (*(versionCtrl+DRIVER_TYPE_LCM)==LCM_RENESAS_R61531_V1)) {
              if((*(versionCtrl+DRIVER_TYPE_LCM)==LCM_TOSHIBA_UPD161809_V1)) {
                  register_lcd_backlight_trigger(lcd_backlight_trigger_toshiba);
              }
               if((*(versionCtrl+DRIVER_TYPE_LCM)==LCM_RENESAS_R61531_V1)) {
                   register_lcd_backlight_trigger(lcd_backlight_trigger_renesas);
              }
              rc = request_irq(dsi_irq, mipi_dsi_isr_cmd, IRQF_DISABLED,
                              "MIPI_DSI", 0);
        }else{   

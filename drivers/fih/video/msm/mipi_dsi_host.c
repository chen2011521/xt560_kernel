...
static int g_lcmboot = 1;
static struct timer_list lcmon_timer;

extern int proc_comm_set_led(unsigned id, unsigned level);

static void boot_lcmboot_on_func(unsigned long unused)
{
	proc_comm_set_led(0,40);
}
...
void mipi_dsi_host_init(struct mipi_panel_info *pinfo)
{
	...
	if (pinfo->mode == DSI_VIDEO_MODE) {
        ...
        printk(KERN_ERR "[DISPLAY] %s g_lcmboot=%d\n", __func__,g_lcmboot);
        if (g_lcmboot) {
            g_lcmboot = 0;
            proc_comm_set_led(0,0);
            setup_timer(&lcmon_timer, boot_lcmboot_on_func, 0);
            mod_timer(&lcmon_timer, jiffies + msecs_to_jiffies(600));
        }
...
/* Set DSI Command Timeout in FTM Only */
#ifndef CONFIG_FIH_FTM
	wait_for_completion(&dsi_dma_comp);
#else
	wait_for_completion_timeout(&dsi_dma_comp, msecs_to_jiffies(1000)); //Ming
#endif
/* } FIH, CHHsieh, 2011/09/18 */
...
/* The backlight should turn on after the DSI_INTR_CMD_MDP_DONE */
#ifdef CONFIG_FIH_7X27A_PROJS

/*when the panel receive initial command, DSI_INTR_CMD_DMA_DONE isr come.
then the panel splash ,DSI_INTR_CMD_MDP_DONE isr come.*/
static boolean command_dma_flag = FALSE;

backlight_trigger_t trigger;

irqreturn_t mipi_dsi_isr_cmd(int irq, void *ptr)
{
	uint32 isr;
    ...
    //printk(KERN_INFO "[DISPLAY] jenny +%s isr=0x%x\n", __func__,isr);
#ifdef CONFIG_FB_MSM_MDP40
	mdp4_stat.intr_dsi++;
#endif
...
void register_lcd_backlight_trigger(backlight_trigger_t backlight_trigger)
{
    trigger = backlight_trigger;
}
#endif
/* } FIHNJDC, SW4-BSP Jenny, 2011/10/12 */

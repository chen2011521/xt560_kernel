...
void mdp_hw_init(void)
{
    ...
    /* FIHNJDC, SW4-BSP Jenny, 2011/12/30{ */
...
/* } FIHNJDC, SW4-BSP Jenny, 2011/12/30 */
	...
	if ((inpdw(MDP_BASE + 0xF0000)) & 0x01) { /* enabled already */
		/* If MIPI was already enabled in APPSBOOT, 
		   we should set MDP_DMA2_BLOCK power-on here to keep MDP power status. 
		   refer to mdp_dma_dsi_video.c */
	    mdp_pipe_ctrl(MDP_DMA2_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
        printk(KERN_INFO "[Display] %s: MIPI enabled already.\n", __func__);
	}

...
void nm32x_62x_tsi_rx_notify(void *data)
{
...
//currently only two ioctl for tigger and stop are provided..
	printk(KERN_DEBUG"TSI cmd is %x\n",cmd);
	switch(cmd)
	{
		case TSI_TRIGGER:
			if(the_dev->state == tsif_state_running)
...
static int tsif_init_one(struct tsif_dev *the_dev, int index)
{
	...
	//pr_info("\n Vincent : %s[%d] %x %x \n", __func__, index,tsif_attach,the_dev->cookie);

	the_dev->cookie = tsif_attach(index, nm32x_62x_tsi_rx_notify, (void *)the_dev);
...
static int nm32x_62x_tsi_probe(struct platform_device *pdev)
{
	...
	printk(KERN_ERR "nm32x_62x_tsi_probe()\n");
	...
	printk(KERN_ERR "nm32x_62x_tsi_probe() -- test %d \n",rc);
	...
	printk(KERN_ERR "nm32x_62x_tsi_probe() -- test 2 %d \n",rc);
...
static int __init nm32x_62x_tsi_init(void *hdl)
{
	printk(nm32x_62x);
...
return  ADD_DYN_PLATFORM_DRIVER(hdl, &nm32x_62x_tsi_driver);
// return platform_driver_register(&nm32x_62x_tsi_driver);
}

DECLARE_DYN_LOADER(DRIVER_TYPE_TSIF_NM32X_62X, TSIF_NM32X_62X_V1, nm32x_62x_tsi_init, LEVEL6);

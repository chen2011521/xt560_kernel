...
static int action_open(struct msm_tsif_device *tsif_device)
{
	int rc = -EINVAL;
	int result;
	//struct msm_tsif_platform_data *pdata =    //lewis
		//tsif_device->pdev->dev.platform_data;
...
static int __init mod_init(void *hdl)
{
	int rc=ADD_DYN_PLATFORM_DRIVER(hdl, &msm_tsif_driver);
	if (rc)
...
DECLARE_DYN_LOADER(DRIVER_TYPE_TSIF_NM32X_62X, TSIF_NM32X_62X_V1, mod_init, LEVEL6);

...
#define OV7692_SLAVER_ADDR                            0x78 >> 1
...
struct reg_addr_val_pair_struct ov7692_init_settings_array[] = {
    ...
    {0xb5, 0x40},
    {0xff, 0x00},
    {0x0c, 0x00}, //0x00    [6] mirror enable  [7] filp enable
    {0x62, 0x10},
    {0x12, 0x00},
    {0x17, 0x65},
    {0x18, 0xa4},
    ...
    {0x31, 0x87}, //0x83 for 30fps, 0x87 for 15fps    
    {0x14, 0x39},   //bit 1 enable auto banding  16x gain

    {0xd2, 0x07}, //bit 1 enable uv                 
    {0xd8, 0x38},  //sat u                
    {0xd9, 0x38},  //sat v    
...
static int32_t ov7692_sensor_setting(int update_type, int rt)
{
    ...
    printk(KERN_ERR "%s  update_type %d\n", __func__,update_type);
...
static int32_t ov7692_power_down(void)
{
   ...
   gpio_request(OV7692_GPIO_PWD,"ov7692_pwd");
    ...
    printk(KERN_ERR "%s GPIO_CFG_direction_output 116--------rc %d\n", __func__,rc);
    msleep(1);
...
static int ov7692_probe_init_sensor(const struct msm_camera_sensor_info *data)
{
	...
	printk(KERN_ERR  "ov7692 model_id = 0x%x, 0x%x, 0x%x\n",
...
int ov7692_sensor_open_init(const struct msm_camera_sensor_info *data)
{
    ...
    printk(KERN_ERR "%s GPIO_CFG_tlmm_config 116--------rc %d\n", __func__,rc);
   ...
   /*powerdown = 0*/
    gpio_direction_output(OV7692_GPIO_PWD, 0);
    mdelay(5);
	...
	ov7692_i2c_write_b_sensor(0xb4, 0xc0);
	ov7692_i2c_write_b_sensor(0xb5, 0x40);
...
{
	int rc = -EBADF;
    printk(KERN_ERR "[OV7692] Enter %s\n", __func__);
	mutex_lock(&ov7692_mut);
	ov7692_i2c_write_b_sensor(0xff, 0x01);
	ov7692_i2c_write_b_sensor(0xb4, 0x40);
	ov7692_i2c_write_b_sensor(0xb5, 0x30);
	ov7692_i2c_write_b_sensor(0xff, 0x00);
	ov7692_power_down();
		...
		struct msm_sensor_ctrl *s)
{
   ...
   /*powerdown = 0*/
	...
	s->s_mount_angle = info->sensor_platform_info->mount_angle;
     ...
     printk(KERN_ERR"ov7692_sensor_probe: SENSOR PROBE DOWN!\n");
    /*powerdown = 1*/
     gpio_direction_output(OV7692_GPIO_PWD, 1);
     mdelay(5);
...
probe_fail:
	...
	printk(KERN_ERR"ov7692_sensor_probe: SENSOR PROBE FAILS!\n");
    /*powerdown = 1*/
    gpio_direction_output(OV7692_GPIO_PWD, 1);
    mdelay(5);
    gpio_free(OV7692_GPIO_PWD); 
...
static int __init ov7692_init(void* hdl)
{
...
static int __init ov7692_init_v2(void* hdl)
{
	return platform_driver_register(&msm_camera_driver);
}
...
DECLARE_DYN_LOADER(DRIVER_TYPE_FRONT_CAM, 
			FRONT_CAM_OV7692_V2, 
			ov7692_init_v2, LEVEL6);

//TBP PR2.5
static int __init ov7692_init_v3(void* hdl)
{
	return platform_driver_register(&msm_camera_driver);
}
...
DECLARE_DYN_LOADER(DRIVER_TYPE_FRONT_CAM, 
			FRONT_CAM_OV7692_V3, 
			ov7692_init_v3, LEVEL6);

//IRMPrime
static int __init ov7692_init_v4(void* hdl)
{
	return platform_driver_register(&msm_camera_driver);
}
DECLARE_DYN_LOADER(DRIVER_TYPE_FRONT_CAM, 
			FRONT_CAM_OV7692_V4, 
			ov7692_init_v4, LEVEL6);

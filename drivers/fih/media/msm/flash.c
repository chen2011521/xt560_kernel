...
static struct i2c_client *sc628a_client = NULL;
		...
		const struct i2c_device_id *id)
{
    ...
    //yisong add for project NPM 
    versionCtrl = read_hw_cfg();
        if(*(versionCtrl+DRIVER_TYPE_FLASH_LIGHT)==FLASH_LIGHT_V3)
           NPM_flash_flag = 1;
	...
	unsigned led_state)
{
    ...
    //yisong add for NPM hardware design
    if(NPM_flash_flag == 1)
    {
        printk("NPM project [FLASHLED] %s: led_state=%d\n", __func__, led_state);
        switch (led_state) {
        case MSM_CAMERA_LED_OFF:
            //sc628a_i2c_write_b_flash(0x02, 0x0);
            sc628a_i2c_write_b_flash(0x00, 0x00);
            sc628a_i2c_write_b_flash(0x01, 0x00);
            gpio_set_value_cansleep(current_driver->led1, 0);
            rc = proc_comm_set_led(PMIC8028_ID_CAMERA_FLASH, 0);
            break;
        case MSM_CAMERA_LED_LOW:
            //sc628a_i2c_write_b_flash(0x02, 0x06);
            printk("[FLASHLED] %s: set 50 percent light for pre-flash\n", __func__);
            rc = proc_comm_set_led(PMIC8028_ID_CAMERA_FLASH, 0);    
            gpio_set_value_cansleep(current_driver->led1, 1);
            sc628a_i2c_write_b_flash(0x00, 0xE0);//50%
            sc628a_i2c_write_b_flash(0x01, 0x30);
            break;
        case MSM_CAMERA_LED_HIGH:
    ...
    else
    {
        ...
        case MSM_CAMERA_LED_OFF:
            ...
            sc628a_i2c_write_b_flash(0x01, 0x00);
            gpio_set_value_cansleep(current_driver->led1, 0);
            gpio_set_value_cansleep(current_driver->led2, 0);
            break;
        case MSM_CAMERA_LED_LOW:
            //sc628a_i2c_write_b_flash(0x02, 0x06);
            printk("[FLASHLED] %s: set 50 percent light for pre-flash\n", __func__);

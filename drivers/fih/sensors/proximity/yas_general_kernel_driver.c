...
extern int yas_ps_suspend(void);
extern int yas_ps_resume(void);
extern int yas_ps_setdelay(int delay);
#endif
extern int ps_raw_data(void);
...
void yas_prox_sensor_notify(int x, int y, int z)
{
    if(this_data){
...
/*Jordan - add ps raw data*/
static ssize_t
sensor_raw_data_show(struct device *dev,
        struct device_attribute *attr,
        char *buf)
{
	int raw_data = -1;
	raw_data = ps_raw_data();
    return sprintf( buf, "%d\n", raw_data );
}
/*--------------------------------*/
...
static DEVICE_ATTR(raw_data, S_IRUGO, sensor_raw_data_show, NULL);

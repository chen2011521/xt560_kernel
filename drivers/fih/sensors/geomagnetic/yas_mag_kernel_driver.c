...
#if 0
#include <mach/vreg.h>
static int vreg_l17_onoff(u8 v)
{
    int rc=0;
    static struct vreg *vreg = NULL;
    
    printk(KERN_ERR "--VREG_L17 on/off start (%d)--\n", v);

    if (vreg == NULL) {
        vreg = vreg_get(NULL, "bt");        
        if (IS_ERR(vreg)) {
            pr_err("%s: vreg_get(%s) failed (%ld)\n",
                   __func__, "bt", PTR_ERR(vreg));
            return -1;
        }

        rc = vreg_set_level(vreg, 2750);
        if (rc) {
    ...
    if (1==v) {
        rc = vreg_enable(vreg);
        if (rc) {
            printk(KERN_ERR "%s: L17 enable failed (%d)\n",
                   __func__, rc);
            return -1;
        }
    }
    ...
    printk(KERN_ERR "--VREG_L17 on/off end--\n");
...
struct geomagnetic_data {
    ...
    struct FIH_geomsensor *fih_geom_data;
};
...
#if TRACE_OUTPUT
            printk("yas_acc_work_func : data(%10d %10d %10d) accuracy(%5d)\n",
                   magdata.xyz.v[0], 
                   magdata.xyz.v[1], 
                   magdata.xyz.v[2], 
                   accuracy);
...
static int get_geomagnetic_hw_threshold(void)
{
    ...
    {
    case 0:       
        return YAS_DEFAULT_MAGCALIB_THRESHOLD; /*tinboost+*/
    default :
        return YAS_DEFAULT_MAGCALIB_THRESHOLD; 
    }
}
...
geomagnetic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    int sysfs_linked = 0, sysfs_raw_linked = 0;
    int data_registered = 0, raw_registered = 0, i;
    struct yas_mag_filter filter;

	printk(KERN_INFO "%s: %s \n", GEOMAGNETIC_I2C_DEVICE_NAME, __func__);
    ...
    data->threshold = get_geomagnetic_hw_threshold();
    data->fih_geom_data = (struct FIH_geomsensor *)client->dev.platform_data;
    ...
    rt = sysfs_create_link(input_data->dev.kobj.parent, 
                            ...
                            input_data->name);
    if (rt) {        
        YLOGE(("geomagnetic_probe: sysfs_create_link failed[%s]\n",
               input_data->name));
        goto err;
    }
    sysfs_linked = 1;
    ...
    input_set_abs_params(input_raw, ABS_RAW_THRESHOLD, 0, 0xffff, 0, 0);
    ...
    rt = sysfs_create_link(input_raw->dev.kobj.parent, 
                            &input_raw->dev.kobj, 
                            input_raw->name);
    if (rt) {        
        YLOGE(("geomagnetic_probe: sysfs_create_link failed[%s]\n",
    ...
    input_set_drvdata(data->input_raw, data);
    ...
    if (hwdep_driver.set_position != NULL) {
        if (hwdep_driver.set_position(data->fih_geom_data->placement) < 0) {
...
err:
    if (data != NULL) {
        if (input_raw != NULL) {
            if(sysfs_raw_linked){
                sysfs_remove_link(input_raw->dev.kobj.parent, 
                                  input_raw->name);
            }
        ...
        if (input_data != NULL) {
            if(sysfs_linked){
                sysfs_remove_link(input_data->dev.kobj.parent, 
                                  input_data->name);
            }
            if (sysfs_created) {
    ...
    printk(KERN_ERR "%s: %s err = %d\n", GEOMAGNETIC_I2C_DEVICE_NAME, __func__, rt);
    return rt;
}
...
geomagnetic_remove(struct i2c_client *client)
{
    ...
    if (data != NULL) {
        ...
        sysfs_remove_link(data->input_raw->dev.kobj.parent, 
                          data->input_raw->name);

        sysfs_remove_link(data->input_data->dev.kobj.parent, 
...
#ifdef CONFIG_PM
static int yas_mag_pm_suspend(struct device *dev) 
{
...
static int yas_mag_pm_resume(struct device *dev) 
{
    struct geomagnetic_data *data = dev_get_drvdata(dev);
...
static struct dev_pm_ops yas_mag_pm_ops = {
    .suspend        = yas_mag_pm_suspend,
    .resume         = yas_mag_pm_resume,
};
#endif
...
/*E: for ftm mode */
static int yas_mag_open(struct inode *inode, struct file *file)
{
    printk(KERN_INFO "[yas_mag]%s\n", __func__);
	return 0;
}

static ssize_t yas_mag_read(struct file *file, char __user *buffer, size_t size, loff_t *f_ops)
{	
    printk(KERN_INFO "[yas_mag]%s\n", __func__);
	return 0; 
}
...
/*ECOMPASS*/ 
#define SENSOR_ECOMPASS_ON	            0x0D00
#define SENSOR_ECOMPASS_OFF		        0x0D01
#define SENSOR_ECOMPASS_READID	        0x0D02
#define SENSOR_ECOMPASS_READXYZ	        0x0D03
#define SENSOR_ECOMPASS_READTEMPERATURE	0x0D04

#define YAS_MAG_CHIP_ID					0x40

static long yas_mag_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	void __user *argp = (void __user *)arg;

    printk(KERN_INFO "[yas_mag] %s : cmd = 0x%x, arg = 0x%x\n", __func__, cmd, (unsigned int)arg);

	switch (cmd)
	{/*GSENSOR*/ 
    case SENSOR_ECOMPASS_ON:
        return hwdep_driver.set_enable(1);
    case SENSOR_ECOMPASS_OFF:
        return hwdep_driver.set_enable(0);
    case SENSOR_ECOMPASS_READID:
        {
            int id = YAS_MAG_CHIP_ID;
        ...
        break;
    case SENSOR_ECOMPASS_READXYZ:
        {
            uint32_t time_delay_ms = 100;
            struct yas_mag_data magdata;
            int rt;

            if (hwdep_driver.measure == NULL) 
            {
                return -EFAULT;
            }
            ...
            if (copy_to_user(argp, &magdata, sizeof(struct yas_mag_data)))
            {
                return -EFAULT;
            }
        }
        break;
    case SENSOR_ECOMPASS_READTEMPERATURE:
        {
            uint32_t time_delay_ms = 100;
            struct yas_mag_data magdata;
            int rt , temperature;
            ...
            temperature = (int) magdata.temperature;
                 
            if (copy_to_user(argp, &temperature, sizeof(int)))
            {
                return -EFAULT;
            }
        }
...
static int yas_mag_release(struct inode *inode, struct file *filp)
{
    printk(KERN_INFO "[yas_mag]%s\n", __func__);
	return 0;
}

static const struct file_operations yas_mag_dev_fops = {
	.owner = THIS_MODULE,
	.open = yas_mag_open,
	.read = yas_mag_read,
	.unlocked_ioctl = yas_mag_ioctl,
	.release = yas_mag_release,
};

static struct miscdevice yas_mag_dev =
{
    .minor = MISC_DYNAMIC_MINOR,
    .name = "compass",             //reserve the file name for FTM can control
    .fops = &yas_mag_dev_fops,
};
/*E: for ftm mode */



/*FIHTDC-DerricKDRLiu-[IRM] Ecompass ftm test function usage {*/ 
#ifdef CONFIG_FIH_7X27A_PROJS
static ssize_t ecompass_yas529_dev_read(struct file *fp, char __user *buf,
                            size_t count, loff_t *pos)
{
	int ret = 0;
	char tmp[64];

	YLOGE("ecompass_yas529_dev_read \n");
	...
	{
		ret = i2c_master_recv(this_client, tmp, count);
		if(copy_to_user(buf, tmp, ret)) 
		{
			ret = -EFAULT;
		}
#if DEBUG		
		YLOGE("result( %d, %d, 0x%02x)\n", count, ret, this_client->addr);
...
static ssize_t ecompass_yas529_dev_write(struct file *fp, const char __user *buf,
                             size_t count, loff_t *pos)
{
	int ret = 0;
	char tmp[64];

	YLOGE("ecompass_yas529_dev_write\n");
...
#endif			
			ret = i2c_master_send(this_client, tmp, count);
		}
...
static struct file_operations ecompass_fileops = {
    .owner      = THIS_MODULE,
    .read        = ecompass_yas529_dev_read,
    .write        = ecompass_yas529_dev_write,
    //.unlocked_ioctl      = ecompass_yas529_dev_ioctl,
};

static struct miscdevice ecompass_device = {
    .minor = MISC_DYNAMIC_MINOR,
    .name  = "Ecompass",
    .fops  = &ecompass_fileops,
};
#endif
/*} FIHTDC-DerricKDRLiu-[IRM] Ecompass ftm test function usage*/  
...
static int __init
geomagnetic_init(void *hdl)
{
    int ret;

    ret = gpio_tlmm_config(GPIO_CFG(18, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), 
                           GPIO_CFG_ENABLE);
    ret = gpio_direction_output(18, 1);
    mdelay(10);

    if((ret=ADD_DYN_I2C_DRIVER(hdl, &geomagnetic_i2c_driver)))
    {
        printk(KERN_ERR "[yas_mag] %s: geomagnetic_i2c_driver driver init fail\n", __func__);
        return ret;
    }

    if((ret=misc_register(&yas_mag_dev)))
	{
        i2c_del_driver(&geomagnetic_i2c_driver);
		printk(KERN_ERR"[yas_mag] %s: yas_mag_dev register failed.\n", __func__);
	}


	/*FIHTDC-DerricKDRLiu-[IRM] Ecompass ftm test function usage {*/ 
#ifdef CONFIG_FIH_7X27A_PROJS
	if ((ret = misc_register(&ecompass_device)) < 0) {
		printk(KERN_ERR "ecompass_device failed[%d]\n", ret);
	}
#endif
	/*} FIHTDC-DerricKDRLiu-[IRM] Ecompass ftm test function usage*/ 
...
DECLARE_DYN_LOADER(DRIVER_TYPE_MSENSOR, MSENSOR_YAS529_V1, geomagnetic_init, LEVEL6);

static int __init geomagnetic_init_v2(void *hdl)
{
    int ret;
...
static int __init geomagnetic_init_v3(void *hdl)
{
     int ret;
...
static int __init geomagnetic_init_v4(void *hdl)
{
     int ret;
...
DECLARE_DYN_LOADER(DRIVER_TYPE_MSENSOR, MSENSOR_YAS529_V2, geomagnetic_init_v2, LEVEL6);
DECLARE_DYN_LOADER(DRIVER_TYPE_MSENSOR, MSENSOR_YAS529_V3, geomagnetic_init_v3, LEVEL6);
DECLARE_DYN_LOADER(DRIVER_TYPE_MSENSOR, MSENSOR_YAS529_V4, geomagnetic_init_v4, LEVEL6);

static void __exit
geomagnetic_term(void)
{
    i2c_del_driver(&geomagnetic_i2c_driver);
	misc_deregister(&yas_mag_dev);
}

...
#define YAS_LIS331DLH_DEFAULT_THRESHOLD                                    20000
...
static int yas_lis331dlh_read_reg(unsigned char, unsigned char *, unsigned char);
static int yas_lis331dlh_write_reg(unsigned char, unsigned char *, unsigned char);
static int yas_lis331dlh_read_reg_byte(unsigned char);
static int yas_lis331dlh_write_reg_byte(unsigned char, unsigned char);
...
static void yas_lis331dlh_init_data(void) {
    ...
    acc_data.threshold = YAS_LIS331DLH_DEFAULT_THRESHOLD;
...
static int yas_lis331dlh_read_reg(unsigned char adr, unsigned char *buf, unsigned char len)
{
...
static int yas_lis331dlh_write_reg(unsigned char adr, unsigned char *buf, unsigned char len)
{
    struct yas_acc_driver_callback *cbk = &pcb->callback;
    int err;
...
static int yas_lis331dlh_write_reg_byte(unsigned char adr, unsigned char val)
{
    return yas_lis331dlh_write_reg(adr, &val, 1);
}
...
#define yas_lis331dlh_update_bits(r,v) \
    yas_lis331dlh_write_reg_byte(r##_REG, ((yas_lis331dlh_read_reg_byte(r##_REG) & ~r##_MASK) | ((v) << r##_SHIFT)))
...
static int yas_lis331dlh_power_up(void)
{
    yas_lis331dlh_write_reg_byte(YAS_LIS331DLH_CTRL_REG1, acc_data.odr | YAS_LIS331DLH_XYZ_ENABLE);
...
static int yas_lis331dlh_power_down(void)
{
    yas_lis331dlh_write_reg_byte(YAS_LIS331DLH_CTRL_REG1, YAS_LIS331DLH_XYZ_ENABLE);
...
static int yas_lis331dlh_init(void)
{
    ...
    yas_lis331dlh_write_reg_byte(YAS_LIS331DLH_CTRL_REG1, YAS_LIS331DLH_XYZ_ENABLE); /* Enable xyz */
    yas_lis331dlh_write_reg_byte(YAS_LIS331DLH_CTRL_REG2, 0);
    yas_lis331dlh_write_reg_byte(YAS_LIS331DLH_CTRL_REG3, 0);
    yas_lis331dlh_write_reg_byte(YAS_LIS331DLH_CTRL_REG4, YAS_LIS331DLH_FS_2G | YAS_LIS331DLH_BDU_ON | YAS_LIS331DLH_BLE_LE);
    ...
    filter.threshold = YAS_LIS331DLH_DEFAULT_THRESHOLD;
...
static int yas_lis331dlh_set_delay(int delay)
{
    ...
    for (i = 1; i < (int)(sizeof(yas_lis331dlh_odr_tbl) / sizeof(struct yas_lis331dlh_odr)) &&
...
static int yas_lis331dlh_data_filter(int data[], int raw[], struct yas_lis331dlh_acceleration *accel)
{
...
static int yas_lis331dlh_measure(int *out_data, int *out_raw)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        data[i] = 0;
        for (j = 0; j < 3; j++) {
            data[i] += raw[j] * yas_lis331dlh_position_map[pos][i][j];
        }
        /* normalization */
        data[i] *= (YAS_LIS331DLH_GRAVITY_EARTH / YAS_LIS331DLH_RESOLUTION);
    }
...
static int yas_set_offset(struct yas_vector *offset)
{
    ...
    if (offset == NULL ||
        offset->v[0] < YAS_LIS331DLH_ABSMIN_2G || YAS_LIS331DLH_ABSMAX_2G < offset->v[0] ||
        offset->v[1] < YAS_LIS331DLH_ABSMIN_2G || YAS_LIS331DLH_ABSMAX_2G < offset->v[1] ||
        offset->v[2] < YAS_LIS331DLH_ABSMIN_2G || YAS_LIS331DLH_ABSMAX_2G < offset->v[2]) {
        return YAS_ERROR_ARG;
    }
...
static int yas_set_filter(struct yas_acc_filter *filter)
{
    ...
    if (filter == NULL || filter->threshold < 0 || filter->threshold > YAS_LIS331DLH_ABSMAX_2G) {
        return YAS_ERROR_ARG;
    }

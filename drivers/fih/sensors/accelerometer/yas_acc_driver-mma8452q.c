...
#define YAS_MMA8452Q_DEFAULT_THRESHOLD                                     20000
...
static int yas_mma8452q_write_reg_byte(unsigned char, unsigned char);
...
static void yas_mma8452q_init_data(void) {
    ...
    acc_data.threshold = YAS_MMA8452Q_DEFAULT_THRESHOLD;
...
static int yas_mma8452q_read_reg(unsigned char adr, unsigned char *buf, unsigned char len)
{
...
static int yas_mma8452q_write_reg_byte(unsigned char adr, unsigned char val)
{
    return yas_mma8452q_write_reg(adr, &val, 1);
}
...
#define yas_mma8452q_update_bits(r,v) \
    yas_mma8452q_write_reg_byte(r##_REG, ((yas_mma8452q_read_reg_byte(r##_REG) & ~r##_MASK) | ((v) << r##_SHIFT)))
...
static int yas_mma8452q_init(void)
{
    ...
    filter.threshold = YAS_MMA8452Q_DEFAULT_THRESHOLD;
...
static int yas_mma8452q_measure(int *out_data, int *out_raw)
{
    ...
    if (yas_mma8452q_read_reg(YAS_MMA8452Q_ACC_REG, buf, 6) != 0) {
        ...
        for (i = 0; i < 3; i++) raw[i] = ((int16_t)((buf[i*2] << 8)) | (buf[i*2+1] & 0xfe)) >> 4;
    }
    ...
    for (i = 0; i < 3; i++) {
        ...
        data[i] = 0;
        for (j = 0; j < 3; j++) {
            data[i] += raw[j] * yas_mma8452q_position_map[pos][i][j];
        }
        /* normalization */
        data[i] *= (YAS_MMA8452Q_GRAVITY_EARTH / YAS_MMA8452Q_RESOLUTION);
    }
...
static int yas_set_offset(struct yas_vector *offset)
{
    ...
    if (offset == NULL ||
        offset->v[0] < YAS_MMA8452Q_ABSMIN_2G || YAS_MMA8452Q_ABSMAX_2G < offset->v[0] ||
        offset->v[1] < YAS_MMA8452Q_ABSMIN_2G || YAS_MMA8452Q_ABSMAX_2G < offset->v[1] ||
        offset->v[2] < YAS_MMA8452Q_ABSMIN_2G || YAS_MMA8452Q_ABSMAX_2G < offset->v[2]) {
        return YAS_ERROR_ARG;
    }

...
#ifdef YAS_ACC_I2C_SLAVEADDR
#undef YAS_ACC_I2C_SLAVEADDR
#endif
static int YAS_ACC_I2C_SLAVEADDR = 0x18;
...
#define YAS_BMA250_DEFAULT_THRESHOLD                                        80000
...
static void yas_bma250_init_data(void) {
    ...
    acc_data.threshold = YAS_BMA250_DEFAULT_THRESHOLD;
...
static int yas_bma250_init(void)
{
    ...
    filter.threshold = YAS_BMA250_DEFAULT_THRESHOLD;
...
static int yas_bma250_set_delay(int delay)
{
    ...
    /*FIH-NJDC-SD3, DK, fix self test fail */
    if(i >= (int)(sizeof(yas_bma250_odr_tbl) / sizeof(struct yas_bma250_odr)))
    {
        i = (int)(sizeof(yas_bma250_odr_tbl) / sizeof(struct yas_bma250_odr));
		i--;
    }

    odr = yas_bma250_odr_tbl[i].odr;
    acc_data.delay = delay;

#if 0
    printk("yas_bma250_set_delay : delay(%10d) odr(%d)\n", delay, odr);
#endif
...
/*  Global function                                                            */
/* --------------------------------------------------------------------------- */
int bma250_yas_acc_driver_init(void *drv, 
							struct FIH_gsensor *acc_data)
{
    struct yas_acc_driver_callback *cbk;
    struct yas_acc_driver *f = (struct yas_acc_driver *)drv;
    ...
    YAS_ACC_I2C_SLAVEADDR = acc_data->i2c_addr; // FIH

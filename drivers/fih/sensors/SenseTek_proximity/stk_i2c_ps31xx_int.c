...
#ifdef DETECT_CM3623
#include "../../../../arch/arm/mach-msm/fih/fih_resources.h"
	int is_cm3623_exist = 1;
...
#ifdef DETECT_CM3623
int Get_is_cm3623_exist(void)
{
	return is_cm3623_exist;
}
#endif
EXPORT_SYMBOL(Get_is_cm3623_exist);
...
static int cm_i2c_read_byte(struct i2c_client *client, int addr, int buf)
{
    ...
    while (retry < 5)
	{
		...
		if (ret > 0)
		{
			INFO("cm3623 i2c read ok!");
			break;
		}
		else
		{
			ERR("cm3623 i2c read byte failed! rc = %d", ret);
			retry ++;
		}
	}
...
static int detect_cm3623(struct i2c_client *client)
{
	...
	INFO("detect_cm3623\n");
	rt = gpio_request(YAS_STK3101_PS_GPIO_OUT, "cm3623_int");
	if (rt)
	{
    ...
    rt = gpio_tlmm_config(GPIO_CFG(YAS_STK3101_PS_GPIO_OUT, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, 
															GPIO_CFG_2MA), GPIO_CFG_ENABLE);
    if(rt) 
	{
        ERR("GPIO%d gpio_tlmm_config=%d", YAS_STK3101_PS_GPIO_OUT, rt);
		gpio_free(YAS_STK3101_PS_GPIO_OUT);
        return -2;
    }
    gpio_direction_input(YAS_STK3101_PS_GPIO_OUT);
	
    while(gpio_get_value(YAS_STK3101_PS_GPIO_OUT) == 0 && retry < 5)
	{
		msleep(20);
		retry ++;
    ...
    if (cm_i2c_read_byte(client, 0x78, 0x20) < 0)
	{
		ERR("There is no CM3623 chip on i2c bus!, check another one(Non-AD)");
	}
	else
	{
		INFO("There is CM3623 chip on i2c bus!");
		gpio_free(YAS_STK3101_PS_GPIO_OUT);
		return 1;
	}

if (cm_i2c_read_byte(client, 0x58, 0x20) < 0)
	{
...
#include "NJ_stk_i2c_ps31xx_int.c"
static struct i2c_driver stk_ps_driver_V1 =
{
    .driver = {
        .name = STKPS_DRV_NAME,
#ifdef CONFIG_PM
...
static int __init stk_i2c_ps31xx_init(void *hdl)
{
	...
	INFO("Entering stk_i2c_ps31xx_init.\n");	
    ...
    if (ret)
        return ret;
	*/	
    if ((ret=ADD_DYN_I2C_DRIVER(hdl, &stk_ps_driver)) < 0) {
        printk(KERN_ERR "[stk3101]%s: ADD_DYN_I2C_DRIVER fail,ret=%d\n", __func__, ret);
		return -EIO;	
    }			
		
    if (pStkPsData == NULL)
	{
	//	printk(KERN_ERR "[stk3101]%s: pStkPsData == NULL\n", __func__);	
        return -EINVAL;
	}
...
static int __init stk_i2c_ps31xx_init_V1(void *hdl)
	{
	
		int ret;	
	
		INFO("Entering stk_i2c_ps31xx_init_V1.\n");	
...
static int __init stk_i2c_ps31xx_init_V2(void *hdl)
	{
	
		int ret;	
	
		INFO("Entering stk_i2c_ps31xx_init_V2.\n");	
		...
		if (ret)
			return ret;
		*/	
		if ((ret=ADD_DYN_I2C_DRIVER(hdl, &stk_ps_driver_V1)) < 0) {
			printk(KERN_ERR "[stk3101]%s: ADD_DYN_I2C_DRIVER fail,ret=%d\n", __func__, ret);
			return -EIO;	
		}			
	...
	/*
		...
		*/
		...
		if (!stk_oss_dev)
		{
		   i2c_del_driver(&stk_ps_driver_V1);
		   return -ENOMEM;
		}
		ADD_DYN_PLATFORM_DEVICE(DECL_VERSION_CTRL(DRIVER_TYPE_PS, 
					STK3101_V2), 
...
DECLARE_DYN_LOADER(DRIVER_TYPE_PS, STK3101_V1, stk_i2c_ps31xx_init_V1, LEVEL6);
DECLARE_DYN_LOADER(DRIVER_TYPE_PS, STK3101_V2, stk_i2c_ps31xx_init_V2, LEVEL6);

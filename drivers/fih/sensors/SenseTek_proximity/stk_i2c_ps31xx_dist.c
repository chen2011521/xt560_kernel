/*
 *  stk_i2c_ps31xx_dist.c - Linux kernel modules for proximity/ambient light sensor
 *  (Polling + Distance Mode)
 *
...
#include "stk_i2c_ps_lookup_table.h"
...
inline void ps_report_event(struct input_dev* dev,int32_t report_value)
{
	...
	input_sync(dev);
	INFO("STK PS : ps input event %d um\n",report_value*10);
}


inline void update_and_check_report_ps(int32_t ps_code)
{
    int32_t ps_distance_last,distance_i;
    ...
    if (distance_i>CONFIG_STK_PS_DISTANCE_HIGH_THRESHOLD)
    {
        ...
        {// From Mid/Short to Long
            ...
            ps_report_event(pStkPsData->ps_input_dev,distance_i);
        }
        else
        { // Still in Long Range
            if (unlikely(abs(distance_i-ps_distance_last)>=CONFIG_STK_PS_CHANGE_THRESHOLD_FAR))
            {
                pStkPsData->ps_distance_last = distance_i;
                ps_report_event(pStkPsData->ps_input_dev,distance_i);
            }
    ...
    { // Mid Range
      // Only Check Dist. Change Threshold

        if (unlikely(abs(distance_i-ps_distance_last)>=CONFIG_STK_PS_CHANGE_THRESHOLD_MID))
        {
            pStkPsData->ps_distance_last = distance_i;
            ps_report_event(pStkPsData->ps_input_dev,distance_i);
         }
    ...
    { // Short Range
      ...
      { // Still in Short Range
        if (unlikely(abs(distance_i-ps_distance_last)>=CONFIG_STK_PS_CHANGE_THRESHOLD_NEAR))
        {
            pStkPsData->ps_distance_last = distance_i;
            ps_report_event(pStkPsData->ps_input_dev,distance_i);
         }
...
static int32_t enable_ps(uint8_t enable)
{
    if (enable)
    {
        ...
        {
            // Dummy Report
            ps_report_event(pStkPsData->ps_input_dev,ps31xx_lookup_table[0]);
            //reset ps reading, and let it generate an input event while enabling
            pStkPsData->ps_distance_last =
            (CONFIG_STK_PS_DISTANCE_HIGH_THRESHOLD+CONFIG_STK_PS_CHANGE_THRESHOLD_FAR+1);
            //reset ps reading, and let it generate an input event while enabling
            pStkPsData->ps_distance_last = ps31xx_lookup_table[0];
...
static ssize_t distance_show(struct kobject * kobj, struct kobj_attribute * attr, char * buf)
{
    int32_t ps_reading;
    STK_LOCK(1);
    ps_reading = pStkPsData->ps_reading;
    STK_LOCK(0);
    return sprintf(buf, "%d um\n", ((uint32_t)ps31xx_lookup_table[ps_reading])*10);
}
...
ssize_t stk_bin_sysfs_read(ps_distance_mode)
{
    ...
    {
        WARNING("STK PS31xx Driver : Error --> Read Distance Mode(bin) size !=1\n");
        return 0;
    }
#endif
                        ...
                        const struct i2c_device_id *id)
{
    ...
    */
    ...
    if (id->driver_data == 0)
    {
        ...
        input_set_abs_params(ps_data->ps_input_dev, ABS_DISTANCE, ps31xx_lookup_table[255],ps31xx_lookup_table[0], 0, 0);

...
static struct usb_string string_defs_moto[] = {
    [0].s       = "Motorola ADB Interface",
    {  },   /* end of list */
};
static struct usb_gadget_strings adb_string_table_moto = {
    .language =     0x0409, /* en-us */
    .strings =      string_defs_moto,
};

static struct usb_gadget_strings *adb_strings_moto[] = {
    &adb_string_table_moto,
    NULL,
};
...
#include "../../fih/usb/gadget/fih_f_adb.c"
...
static int adb_bind_config(struct usb_configuration *c)
{
    ...
    /* +++ FIHTDC, AlbertYCFang, 2011.10.18 +++ */
    if (isMoto)
        dev->function.strings = adb_strings_moto;
    else
	...
	ret = create_root_switch_attribute(dev);
	if (ret<0)
	{
		pr_err("%s: Create attribute \"root\" failed!! <%d>", __func__, ret);
		goto err1;
	}
	/* FIHTDC, Div2-SW2-BSP, DerrickDRLiu, ADB ROOT switch{ */
	return 0;

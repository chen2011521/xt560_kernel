...
#ifdef CONFIG_FIH_USB_PORTING//StevenCPHuang_20110903,include the fih_android.c for HWID control
#include "../../fih/usb/gadget/fih_msm72k_udc.h"
#endif
...
struct msm_endpoint {
	...
	*/
	...
	struct timer_list prime_timer;//1090_new add
};

/* PHY status check timer to monitor phy stuck up on reset */
static struct timer_list phy_status_timer;

static void ept_prime_timer_func(unsigned long data);//1090_new add
...
#define EPT_PRIME_CHECK_DELAY	(jiffies + msecs_to_jiffies(1000))//1090_new add
...
struct usb_info {
	...
	*/
	...
	unsigned prime_fail_count;//1090_new add
...
#ifndef CONFIG_FIH_USB_PORTING //StevenCPHuang_20110905,1090 porting from 1070
...
#ifdef CONFIG_FIH_USB_PORTING//StevenCPHuang_20110903,include the fih_android.c for HWID control
#include "../../fih/usb/gadget/fih_msm72k_udc.c"
#endif

/*FIH-NJ-BSP,2011/11/11. Jerry decrease this value from 1800 to 750.
  because TBP have HW OCP(1A). For IRM later will be set to 1A. +{*/
#define USB_WALLCHARGER_CHG_CURRENT 750
/*FIH-NJ-BSP,Jerry, 2011/11/11.  +}*/

/* FIH, IvanCHTang, 2011/10/25 { */
/* [IRM], Fixed Car kit fail - [4] */
#define USB_CAR_CHG_CURRENT 500
/* } FIH, IvanCHTang, 2011/10/25 */
...
static void usb_chg_detect(struct work_struct *w)
{
    ...
    /*FIH-NJ-BSP,Jerry mark the following code for wall charger suspend issue.2011/11/11 +{*/
	/*if (temp == USB_CHG_TYPE__WALLCHARGER) {
...
static void init_endpoints(struct usb_info *ui)
{
	...
	for (n = 0; n < 32; n++) {
		...
		setup_timer(&ept->prime_timer, ept_prime_timer_func,
			(unsigned long) ept);//1090_new add
...
static void usb_ept_start(struct msm_endpoint *ept)
{
	 ...
	 */
	writel_relaxed(n, USB_ENDPTPRIME);
	mod_timer(&ept->prime_timer, EPT_PRIME_CHECK_DELAY);//1090_new add
}
...
static void handle_endpoint(struct usb_info *ui, unsigned bit)
{
	...
	*/
	...
	del_timer(&ept->prime_timer);//1090_new add
...
static ssize_t debug_prime_fail_read(struct file *file, char __user *ubuf,
				 size_t count, loff_t *ppos)//1090_new add
{
	struct usb_info *ui = file->private_data;
	char kbuf[10];
	size_t c = 0;
...
static void usb_debugfs_init(struct usb_info *ui)
{
	...
	debugfs_create_file("release_wlocks", 0664, dent, ui,
						&debug_wlocks_ops);
	debugfs_create_file("prime_fail_countt", 0220, dent, ui,
						&prime_fail_ops);//1090_new add
}
#else
static void usb_debugfs_init(struct usb_info *ui) {}
#endif
...
static int msm72k_dequeue(struct usb_ep *_ep, struct usb_request *_req)
{
	...
	del_timer(&ep->prime_timer);//1090_new add
	/* Stop the transfer */
	do {
...
/* Add Virtual file for ftm command DISCONNECTUSB */
...
static ssize_t msm72k_reset_usb(struct device *dev,
                     struct device_attribute *attr,
                     char *buf)
{
        struct usb_info *ui = the_usb_info;
...
#ifdef CONFIG_FIH_FTM
extern int proc_comm_config_chg_current(bool on, int curr);	// DIV2-SW2-BSP-IRM-FTM-CHG
#endif
...
// DIV2-SW2-BSP-IRM-FTM-CHG+[	
	...
	case 0:
		on = false;
		ret = proc_comm_config_chg_current(on, 0);
		break;
	case 1:
		on = true;
		ret = proc_comm_config_chg_current(on, 0);
	...
	case 750:
	case 1000:
		ret = proc_comm_config_chg_current(on, mA);
	}
#endif
// DIV2-SW2-BSP-IRM-FTM-CHG+]
	if(ret != 0)
...
static DEVICE_ATTR(reset_usb, 0664, msm72k_reset_usb, NULL);
#endif
/*} FIH, CHHsieh, 2011/08/16 */
...
/* Add Virtual file for ftm command DISCONNECTUSB */
	...
	if (retval != 0)
		dev_err(&ui->pdev->dev, "failed to create sysfs entry:"
			"(dev_attr_turnoffUSB) error: (%d)\n", retval);
    retval = device_create_file(&ui->gadget.dev, &dev_attr_reset_usb);
    if (retval != 0)
        dev_err(&ui->pdev->dev, "failed to create sysfs entry:"
            "(dev_attr_reset_usb) error: (%d)\n", retval);
#endif
/*} FIH, CHHsieh, 2011/08/16 */

...
// Add by Leok 2012/02/27 erlarge Max package size
//#define GS_NOTIFY_MAXPACKET		10	/* notification + 2 bytes */
#define GS_NOTIFY_MAXPACKET		64
// Add by Leok 2012/02/27
#endif
/*-------------------------------------------------------------------------*/
...
extern bool isMoto;
static struct usb_string gser_string_defs_moto[] = {
    [0].s = "Motorola Communication Interface", 
    {  } /* end of list */
};

static struct usb_gadget_strings gser_string_table_moto = {
    .language =     0x0409, /* en-us */
    .strings =      gser_string_defs_moto,
};

static struct usb_gadget_strings *gser_strings_moto[] = {
    &gser_string_table_moto,
    NULL,
};
/* --- FIHTDC, AlbertYCFang, 2011.10.18 --- */
...
#ifdef CONFIG_MODEM_SUPPORT
/* Add by NJDU-SD4 Leok 2012/04/09 for TBE.B-1768
 * get modem interface index in interface list
 */
static int gser_get_intfid(struct usb_composite_dev *cdev, unsigned w_value)
{
	...
	list_for_each_entry(c, &cdev->configs, list){
		...
		if (w_value == 0){
			list_for_each_entry(f, &c->functions, list){
				// ignore disable interface
				if (!f->disabled)
					intfid = i++;
		
				// if find the modem interface then return
...
// add by NJDU-SD4 Leok for TE.B-1768 
		...
		void *data, unsigned length)
{
	...
	// Modify by NJDU-SD4 Leok 2012/04/09 for TBE.B-1768
	gser_intfid = gser_get_intfid(cdev, notify->wValue);
	notify->wIndex = cpu_to_le16(gser_intfid);
...
int __init gser_bind_config(struct usb_configuration *c, u8 port_num)
{
    ...
    /* +++ FIHTDC, AlbertYCFang, 2011.10.18 +++ */
    if (gser_string_defs[0].id == 0 && gser_string_defs_moto[0].id == 0) {
        status = usb_string_id(c->cdev);
        if (status < 0)
            return status;
        if (isMoto)
            gser_string_defs_moto[0].id = status;
        else
    ...
    if (isMoto)
        gser->port.func.strings = gser_strings_moto;
    else
        gser->port.func.strings = gser_strings;
    /* --- FIHTDC, AlbertYCFang, 2011.10.18 --- */ 

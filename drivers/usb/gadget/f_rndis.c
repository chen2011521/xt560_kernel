...
extern bool isMoto;
static struct usb_string rndis_string_defs_moto[] = {
    [0].s = "Motorola Networking Interface",
    [1].s = "Motorola Networking Interface",
    {  } /* end of list */
};

static struct usb_gadget_strings rndis_string_table_moto = {
    .language =     0x0409, /* en-us */
    .strings =      rndis_string_defs_moto,
};

static struct usb_gadget_strings *rndis_strings_moto[] = {
    &rndis_string_table_moto,
    NULL,
};
/* --- FIHTDC, AlbertYCFang, 2011.10.18 --- */
...
rndis_bind_config(struct usb_configuration *c, u8 ethaddr[ETH_ALEN])
{
	...
	if (rndis_string_defs[0].id == 0) {
        ...
        if (isMoto)
            rndis_string_defs_moto[0].id = status;
        else
            rndis_string_defs[0].id = status;
        /* --- FIHTDC, AlbertYCFang, 2011.10.18 --- */
        ...
        if (isMoto)
            rndis_string_defs_moto[1].id = status;
        else
            rndis_string_defs[1].id = status;
        /* --- FIHTDC, AlbertYCFang, 2011.10.18 --- */
    ...
    if (isMoto)
        rndis->port.func.strings = rndis_strings_moto;
    else
        rndis->port.func.strings = rndis_strings;
    /* --- FIHTDC, AlbertYCFang, 2011.10.18 --- */

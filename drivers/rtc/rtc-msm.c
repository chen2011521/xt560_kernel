...
#if 1
	printk(KERN_INFO "msmrtc_timeremote_set_time %s[%d] : %.2u/%.2u/%.4u %.2u:%.2u:%.2u (%.2u)\n",
	       current->comm, (unsigned)current->pid,
...
msmrtc_probe(struct platform_device *pdev)
{
	...
	rc = create_rtcset_attribute(pdev);
	if (rc < 0)
	{
...
msmrtc_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	if (rtc_pdata->rtcalarm_time) {
		...
		/* FIH, IvanCHTang, 2011/10/7 { */
		/* [IRM], Polling function of battery info. - [3] */
		/* In suspend mode, updating battery info. every 10 min.*/
		#define SUSPEND_UPDATE_TIME 600
		if (diff>SUSPEND_UPDATE_TIME) {
			diff=SUSPEND_UPDATE_TIME;
		}
		...
		/* FIH, IvanCHTang, 2011/10/7 { */
		/* [IRM], Polling function of battery info. - [3] */
		msm_pm_set_max_sleep_time((int64_t)((int64_t) SUSPEND_UPDATE_TIME * NSEC_PER_SEC));
		//msm_pm_set_max_sleep_time(0);
		/* } FIH, IvanCHTang, 2011/10/7 */
	}
	atomic_inc(&suspend_state.state);

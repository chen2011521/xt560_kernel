...
// SW251-RL-ChargingCurrentByNV--[
/*--------------------------------------------------------------------------* 
 * Function    : ReadNVByteGroup2
 *
 * Description :
 *     Read the value from the localcfg by the index.
 *
 * Parameters  :
 *
 *     bid    - a byte id between 4 ~ 8.
 *     bvalue - the value according to the choosed byte id returned.
 *
 * Return value: Integer
 *     Zero     - Successful
 *     Not zero - Fail
 *--------------------------------------------------------------------------*/
int ReadNVByteGroup2(unsigned int bid,unsigned int *bvalue)
{
	int ret = 1;
	
	// check the index is inside the reasonable range.
	if ( bid > DEBUG_CHARGING_CERTIFICATION_TEST_ITEM_START || bid < MAX_DEBUG_CHARGING_CERTIFICATION_TEST_ITEM_END)
	{
    *bvalue = localcfg[bid];
    printk(KERN_ERR 
    			  "[dbgcfgtool] %s() LINE:%d, localcfg: 0x%x 0x%x 0x%x 0x%x\n",
    			  __func__, __LINE__,localcfg[4], localcfg[5], localcfg[6], localcfg[7]);		
		ret = 0;
	}
	return ret;
}
...
/*--------------------------------------------------------------------------* 
 * Function    : ReadBatThresFlagNV
 *
 * Description :
 *     Read the value from the nv item to represent the new low battery
 *     threshold.
 * Parameters  :
 *     bvalue    - Battery threshold Flag: 
 										1: Battery threshold flag has enable by nv item.
 										2: default behavior
 *
 * Return value: Integer
 *     Zero     - Successful
 *     Not zero - Fail
 *--------------------------------------------------------------------------*/
int ReadBatThresFlagNV(unsigned int *bvalue)
{
    int ret = 1;
    if(dbgcfg_data_has_initialized)
        ret = DbgCfgGetByBit(DEBUG_POWER_ON_BATT_THRESHOLD, bvalue);

    if(ret)
        printk(KERN_ERR "[dbgcfgtool] %s() LINE:%d, ReadBatThresFlagNV.\n", __func__, __LINE__);
...
/*
 * ReadUartNV() return the 0th bit. NV 7995 has 4 32-bit integers to store debug setting.
 *
 * parameter: bvalue - pointer to variable storing 0th bit
 *
 * return value: 0 - success
 *               1 - error
 *
 * */
int ReadUartNV(unsigned int *bvalue)
{
    int ret = 1;

    if(dbgcfg_data_has_initialized)
        ret = DbgCfgGetByBit(DEBUG_UARTMSG_CFG, bvalue);

    if(ret)
	printk(KERN_ERR "[dbgcfgtool] %s() LINE:%d, DEBUG_UARTMSG_CFG.\n", __func__, __LINE__);
...
static int __init dbgcfgtool_init(void)
{
    ...
    if( ret == 0 )
    {
        ...
        {
            printk(KERN_ERR "[dbgcfgtool] %s() LINE:%d, Initialize dbgcfg data\n", __func__, __LINE__);
            ...
            if ( ret == 0 )
            {
                printk(KERN_ERR "[dbgcfgtool] %s() LINE:%d, Initialize dbgcfg data successful.\n", __func__, __LINE__);
            }
            else
            {
        ...
        {
            memcpy(localcfg,tempcfg,FIH_DEBUG_CFG_LEN);
            printk(KERN_ERR "[dbgcfgtool] %s() LINE:%d, Check sum is correct.\n", __func__, __LINE__);
        }
        else
        {

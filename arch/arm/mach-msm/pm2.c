...
#include "fih/pmdbg.h"
int g_fpmdbg = FPMDBG_INIT_VALUE;
module_param_named(
	fpmdbg, g_fpmdbg, int, S_IRUGO | S_IWUSR | S_IWGRP
);
...
void smsm_pmlog_sleep_info(void)
{
	...
	PM_LOG("[PM] WR(%x):%4ds, wakeup by", wr, convert_sec(msm_pm_smem_data->sleep_time));
	...
	{
		PM_LOG(" GPIO%d", msm_pm_smem_data->reserved2);		/* gpioInfo */
	}
	PM_LOG("\n");
}
//Div2-SW2-BSP-pmlog, HenryMCWang -
...
//Div2-SW2-BSP-pmlog, HenryMCWang +
	...
	if (!from_idle) {
		if (exeCollapse)
			smsm_pmlog_sleep_info();
		printk(KERN_INFO "[PM] msm_pm_power_collapse aborted : collapsed(%d) - ret(%d)\n", collapsed, ret);
	}
#endif	// CONFIG_FIH_SUSPEND_RESUME_LOG
//Div2-SW2-BSP-pmlog, HenryMCWang -
	return ret;
...
/* FIHTDC, Div2-SW2-BSP, Penho, SuspendLog { */
#ifdef CONFIG_FIH_SUSPEND_RESUME_LOG
		gpio_debug_print_state();
		vreg_debug_print_state();
#endif	// CONFIG_FIH_SUSPEND_RESUME_LOG
/* } FIHTDC, Div2-SW2-BSP, Penho, SuspendLog */
		...
		if(cpu_loading_debug_get() == true) {
			if(id == MSM_PM_STAT_SUSPEND) {
				//eventlog("                 P  %lld.%09u\n", s, ns);
				printk(KERN_INFO "[eventlog]                  P  %lld.%09u\n", s, ns);
			} else {
				//eventlog("                 PF %lld.%09u\n", s, ns);
				printk(KERN_INFO "[eventlog]                  PF %lld.%09u\n", s, ns);
			}
		}
		//eventlog_time = 1;
		// FIHTDC, HenryMCWang, 2011.11.03, for event log, ---
	...
	(struct notifier_block *this, unsigned long code, void *_cmd)
{
	if ((code == SYS_RESTART) && _cmd) {
		...
		// FIHTDC, DerrickDRLiu, give restart reason = 0x46544443 when kernel panic reboot {
		} else if (!strcmp(cmd, "panic")) {
			restart_reason = 0x46544443;
		// } FIHTDC, DerrickDRLiu, give restart reason = 0x46544443 when kernel panic reboot
		}
		// FIHTDC, HenryMCWang, give restart reason = 0x77665503 when framework reboot {
		else if (!strcmp(cmd, "exception in system process")){
			restart_reason = 0x77665503;
		}
		// } FIHTDC, HenryMCWang, give restart reason = 0x77665503 when framework reboot
        #ifdef CONFIG_FIH_USB_PORTING
                   else if(!strcmp(cmd, "download")) {//StevenCPHuang-20110812,porting for L3 download++ 
		    restart_reason = 0x444C4F41;//0x46544444;//StevenCPHuang-20110812,porting for L3 download--
                   }else if(!strcmp(cmd, "ftm")) {//StevenCPHuang_20110820,add Moto's RSDLite use 

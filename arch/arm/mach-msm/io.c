...
static struct map_desc msm_io_desc[] __initdata = {
	...
	{
		.virtual =  (unsigned long) MSM_DEBUGMASK_BASE,
		.pfn =      __phys_to_pfn(MSM_DEBUGMASK_PHYS),
		.length =   MSM_DEBUGMASK_SIZE,
		.type =     MT_DEVICE,
	},
	/*FIH, KanyYJLin, debugmask, 2011/09/22{*/
	// FIHTDC, HenryMCWang, 2011.10.31, for new hardware table, +++
	{
		.virtual =  (unsigned long) MSM_HW_TABLE_BASE,
		.pfn =      __phys_to_pfn(MSM_HW_TABLE_PHYS),
		.length =   MSM_HW_TABLE_SIZE,
		.type =     MT_DEVICE,
	},
	// FIHTDC, HenryMCWang, 2011.10.31, for new hardware table, ---
};

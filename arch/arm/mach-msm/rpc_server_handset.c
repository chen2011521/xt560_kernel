...
// Div2-SW5-BSP-Audio-Headset-00+[
{
    ...
    printk(KERN_INFO "[KS]key = %d, key_code = %d, key_parm = %d\n", key, key_code, key_parm);
	switch (key) {
	case KEY_POWER:
/* FIHTDC, Div2-SW2-BSP, Penho, SuspendLog { */
#ifdef CONFIG_FIH_SUSPEND_RESUME_LOG
		printk(KERN_INFO "[PM] POWER key %s\n", ((key_code != HS_REL_K) ? "down" : "up"));
	...
	case SW_HEADPHONE_INSERT_W_MIC:
		hs->mic_on = hs->hs_on = (key_code != HS_REL_K) ? 1 : 0;
            ...
            if(!project_id){
                    ...
                    if(key_parm != HS_REL_K){
                        switch_set_state(&hi->sdev, 1);
                        input_sync(hi->hs_input);            
                    }else{
                        switch_set_state(&hi->sdev, 0);
                        input_sync(hi->hs_input);
                    }
            }
...
/* Porting 1090 */
// Div2-SW5-BSP-Audio-Headset-00+[
/* Don't use the same name as FIH_Headset */
	hs->sdev.name	= "rpc_handset";	
// Div2-SW5-BSP-Audio-Headset-00+[
...
//TBP & TINQ
int trout_h2w_rpc_init_v2(void* hdl)
{
    project_id = 1;

    printk(KERN_INFO"trout_h2w_rpc_init_v2\n");
    return 0;
}

/*Vincent added for ITV HooKKey : PMIC  Headset detect:1  B*/
int trout_h2w_init_rpc_v3(void* hdl)
{
    project_id = 1;

    printk(KERN_INFO"trout_h2w_init_rpc_v3\n");
    return 0;
}

int trout_h2w_init_rpc_v4(void* hdl)
{
    project_id = 1;

    printk(KERN_INFO"trout_h2w_init_rpc_v4\n");
    return 0;
}
...
static int __init hs_init(void)
{
    add_dyn_module(DECL_VERSION_CTRL(DRIVER_TYPE_HEADSET, 
                                     HEADSET_V2), 
                   "HEADSET_V2", 
                   trout_h2w_rpc_init_v2, 
                   __FILE__, __LINE__);

    add_dyn_module(DECL_VERSION_CTRL(DRIVER_TYPE_HEADSET, 
                                     HEADSET_V3), 
                   "HEADSET_V3", 
                   trout_h2w_init_rpc_v3, 
                   __FILE__, __LINE__);

    add_dyn_module(DECL_VERSION_CTRL(DRIVER_TYPE_HEADSET, 
                                     HEADSET_V4), 
                   "HEADSET_V4", 
                   trout_h2w_init_rpc_v4, 
                   __FILE__, __LINE__);

...
#define VREG(_name, _id, _status, _refcnt) \
	{ .name = _name, .id = _id, .status = _status, .refcnt = _refcnt, .mv = 0 }
...
void vreg_debug_print_state(void)
{
	int i, nh = 1;

	for (i = 0; i < ARRAY_SIZE(vregs); i++) {
		if (g_fpmdbg & (DB_VREG_CNT | DB_VREG_MMV | DB_VREG_STA))
			if ((vregs[i].refcnt) ||
				((g_fpmdbg & DB_VREG_MMV) && (vregs[i].mv)) ||
				((g_fpmdbg & DB_VREG_STA) && (vregs[i].status))) {
				if (nh) {printk(KERN_INFO " #     name    mv  refcnt status\n"); nh = 0;}
				printk(KERN_INFO " %2d%10s%5d%5d%7d\n", vregs[i].id, vregs[i].name, vregs[i].mv, vregs[i].refcnt, vregs[i].status);
			}
	}
}
#endif	// CONFIG_FIH_SUSPEND_RESUME_LOG

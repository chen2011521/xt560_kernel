...
/* FIHTDC, Div2-SW2-BSP, Penho, clktracker { */
#include "fih/pmdbg.h"
/* } FIHTDC, Div2-SW2-BSP, Penho, clktracker */
...
#ifdef TRACE_CLK_DBGN
static u32 debug_suspend = 2;
#else	// TRACE_CLK_DBGN
static u32 debug_suspend;
#endif	// TRACE_CLK_DBGN
/* } FIHTDC, Div2-SW2-BSP, Penho, SuspendLog */
...
/* FIHTDC, Div2-SW2-BSP, Penho, SuspendLog { */
	...
	if (likely(!debug_suspend) && ((g_fpmdbg & DB_CLK_STATE) == 0))
...
/* FIHTDC, Div2-SW2-BSP, Penho, SuspendLog { */
#ifdef CONFIG_FIH_SUSPEND_RESUME_LOG
		if ((debug_suspend == 2) || (g_fpmdbg & DB_CLK_STATE)) {
			if (clk && clk->count) {
				pr_info("(%3d)%25s -%15s - %2d - %s", i, msm_clocks[i].con_id, clk->dbg_name, clk->count, msm_clocks[i].dev_id);
				if (clk_get_parent(clk)) {
					struct clk *parent = clk_get_parent(clk);
					printk(" : P(%s)", parent->dbg_name);
				}
				printk("\n");
				cnt++;
			}

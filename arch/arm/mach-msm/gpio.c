...
void gpio_debug_print_state(void)
{
	int i;

	if (g_fpmdbg & DB_GPIO_STATE) {
		printk(KERN_INFO " #     out       oe        in        dif\n");
		for (i = 0; i < ARRAY_SIZE(msm_gpio_chips); i++) {
			unsigned int out = *(volatile unsigned int __force   *)msm_gpio_chips[i].regs.out;
			unsigned int oe = *(volatile unsigned int __force   *)msm_gpio_chips[i].regs.oe;
			unsigned int in = *(volatile unsigned int __force   *)msm_gpio_chips[i].regs.in;
			printk(KERN_INFO " %d%10x%10x%10x%10x\n", i, out, oe, in, ((out ^ in) & oe));
		}
	}
}

...
extern unsigned int debug_rpcmsg_enable;  //FIH-DerrickDRLiu-DbgCfgTool-00+
	...
	if (rq->type == 0) {
		...
		if (!reply) {
			printk(KERN_ERR
				...
				be32_to_cpu(rq->xid));  //FIHTDC-DerrickDRLiu-DbgCfgTool-00*
			return -EINVAL;
		}
...
{
	// PATCH, msm: rpc: While powering down flush all the rpcrouter workqueues, 20111113, +++
